...
| `backends.radio.enabled`                      | `false`                         | Internet radio; TuneIn (§12) is currently dropped from the image pending Mopidy-4 compatibility (§13). Enabling this seeds a default 100-station playlist (real, verified stream URLs from radio-browser.info's public API, ranked by its own vote count) into Mopidy's `[m3u]` playlist directory on first run, browsable/playable in the web client like any other playlist — a real, if smaller, substitute for TuneIn's station directory. Requires internet connectivity, no credentials needed                                                                                               |
