...
- `package.json` had no `"files"` field, so the npm tarball shipped the
  entire `src/` and `test/` trees alongside `dist/` (177 files, 246KB)
  -- it worked only because npm's gitignore-fallback packing happened to
  include the gitignored `dist/` anyway, not because anything declared
  it. Added `"files": ["dist"]` so only the built output ships (110
  files, 96KB).
...
- Restyled the zone mute/delete controls in the webapp: small circular icon
  buttons (🔊/🔇, 🗑), visually distinct from the pill-shaped source-selection
  buttons, right-aligned on the same row as those buttons. Removed the
  zone-top source badge, redundant with the source buttons' own "on" state.
  Volume slider/percentage moved to their own row below the button row.
